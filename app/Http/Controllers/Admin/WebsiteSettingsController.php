...
class WebsiteSettingsController extends Controller
{
    ...
    {
        $settings = WebsiteSetting::getSettings();
        return view('admin.website-settings.header-footer', compact('settings'));
    }
    ...
    public function updateHeaderFooter(Request $request)
    {
        $validated = $request->validate([
            ...
            'support_button_text' => ['nullable', 'string', 'max:50'],
            'support_button_url' => ['nullable', 'string', 'max:255'],
            'footer_section_1_title' => ['nullable', 'string', 'max:50'],
            'footer_section_2_title' => ['nullable', 'string', 'max:50'],
            'footer_section_3_title' => ['nullable', 'string', 'max:50'],
            'facebook_url' => ['nullable', 'url', 'max:255'],
            'twitter_url' => ['nullable', 'url', 'max:255'],
            'linkedin_url' => ['nullable', 'url', 'max:255'],
        ...
        if ($request->hasFile('logo')) {
            ...
            // Store in public/uploads/website/logo
            $logoPath = public_path('uploads/website/logo');
            if (!file_exists($logoPath)) {
                mkdir($logoPath, 0755, true);
            }
        ...
        return redirect()->route('admin.website-settings.header-footer')
            ->with('success', 'Header & Footer settings updated successfully!');
    }
}
