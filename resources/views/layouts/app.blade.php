...
<head>
    ...
    <title>
        @yield('title', 'CISA Interdisciplinary Journal (CIJ) - A Platform for Interdisciplinary Research Excellence')
    </title>
...
<body class="bg-white min-h-screen antialiased">
    ...
    <nav class="bg-white/95 backdrop-blur-md shadow-lg border-b border-gray-100 sticky top-0 z-50 transition-all duration-300"
        x-data="{ mobileOpen: false, scrolled: false }" @scroll.window="scrolled = window.scrollY > 20">
                ...
                <div class="flex items-center space-x-4 sm:space-x-6 md:space-x-8 flex-1 min-w-0">
                    <div class="flex-shrink-0">
                        <a href="{{ route('journals.index') }}"
                            class="flex items-center no-underline hover:no-underline group">
                            @php
                            ...
                            @if($logoUrl)
                                <img src="{{ $logoUrl }}" alt="CISA Interdisciplinary Journal Logo"
                                ...
                                <div class="w-14 h-14 sm:w-16 sm:h-16 md:h-20 md:w-20 lg:h-24 lg:w-24 bg-gradient-to-br from-purple-600 to-purple-800 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg"
                                    style="display:none;">
                                    <i class="fas fa-book-open text-white text-xl sm:text-2xl md:text-3xl"></i>
                                </div>
                            @else
                                <div
                                    class="w-14 h-14 sm:w-16 sm:h-16 md:h-20 md:w-20 lg:h-24 lg:w-24 bg-gradient-to-br from-purple-600 to-purple-800 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg transition-transform duration-300 group-hover:scale-105">
                                    <i class="fas fa-book-open text-white text-xl sm:text-2xl md:text-3xl"></i>
                                </div>
                            @endif
                        </a>
                    ...
                    <div class="hidden lg:flex items-center space-x-1">
                        <a href="{{ route('journals.index') }}"
                            class="px-4 py-2 text-sm font-semibold text-purple-700 rounded-lg hover:bg-purple-50 transition-all duration-200 relative">
                            <span class="relative z-10">{{ __('common.home') }}</span>
                            <span class="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-600 rounded-full"></span>
                        </a>
                        <a href="{{ route('publish.index') }}"
                            class="px-4 py-2 text-sm font-semibold text-gray-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 transition-all duration-200">
                            {{ __('common.publish_with_us') }}
                        </a>
                        <a href="{{ route('journals.index') }}#journals"
                            class="px-4 py-2 text-sm font-semibold text-gray-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 transition-all duration-200">
                            {{ __('common.journals') }}
                        </a>
                        <a href="{{ route('journals.index') }}#announcements"
                            class="px-4 py-2 text-sm font-semibold text-gray-700 rounded-lg hover:bg-purple-50 hover:text-purple-700 transition-all duration-200">
                            Announcements
                        </a>
                    </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="hidden lg:block relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            class="flex items-center space-x-1.5 px-3 py-2 text-sm font-medium text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200 border border-gray-200 hover:border-purple-200">
                            <i class="fas fa-globe text-xs"></i>
                            <span class="uppercase text-xs font-semibold">{{ app()->getLocale() }}</span>
                            <i class="fas fa-chevron-down text-xs"></i>
                        </button>
                        <div x-show="open" @click.away="open = false" x-transition
                            class="absolute right-0 mt-2 w-44 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
                            <a href="{{ route('language.switch', ['locale' => 'en']) }}"
                                class="flex items-center px-4 py-2.5 hover:bg-purple-50 text-gray-700 hover:text-purple-700 transition-colors border-b border-gray-50 last:border-0">
                            ...
                            <a href="{{ route('language.switch', ['locale' => 'pt']) }}"
                                class="flex items-center px-4 py-2.5 hover:bg-purple-50 text-gray-700 hover:text-purple-700 transition-colors border-b border-gray-50 last:border-0">
                                <span class="mr-3 text-lg">🇵🇹</span> <span class="font-medium">Português</span>
                            </a>
                            <a href="{{ route('language.switch', ['locale' => 'sw']) }}"
                                class="flex items-center px-4 py-2.5 hover:bg-purple-50 text-gray-700 hover:text-purple-700 transition-colors">
                        ...
                        <div class="hidden lg:flex items-center space-x-2">
                            @if(auth()->user()->role === 'admin')
                                <a href="{{ route('admin.dashboard') }}"
                                    class="px-4 py-2 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200">
                                    Dashboard
                                </a>
                            ...
                            @elseif(auth()->user()->role === 'copyeditor')
                                <a href="{{ route('copyeditor.dashboard') }}"
                                    class="px-4 py-2 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200">
                                    Dashboard
                                </a>
                            @elseif(auth()->user()->role === 'proofreader')
                                <a href="{{ route('proofreader.dashboard') }}"
                                    class="px-4 py-2 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200">
                                    Dashboard
                                </a>
                            @elseif(auth()->user()->journals()->wherePivotIn('role', ['journal_manager', 'editor', 'section_editor'])->wherePivot('is_active', true)->exists())
                            ...
                            <form method="POST" action="{{ route('logout') }}" class="inline">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2 text-sm font-semibold text-gray-700 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200">
                                    Logout
                                </button>
                            </form>
                        </div>
                    @else
                        <div class="hidden lg:flex items-center space-x-3">
                            ...
                            <a href="{{ route('register') }}"
                                class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-5 py-2 rounded-lg font-semibold text-sm shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105">
                                Get Started
                            </a>
                        </div>
                    @endauth
                    <div class="lg:hidden">
                        <button
                            class="p-2 text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200"
        ...
        <div class="lg:hidden bg-white/98 backdrop-blur-md shadow-xl border-t border-gray-100" x-show="mobileOpen"
            x-transition @click.away="mobileOpen = false">
            <div class="px-4 py-4 space-y-2">
                ...
                <div class="border-b border-gray-100 pb-4 mb-4">
                    ...
                    <div class="grid grid-cols-5 gap-2">
                        <a href="{{ route('language.switch', ['locale' => 'en']) }}"
                            class="px-2 py-2 text-xs rounded-lg text-center font-semibold transition-all duration-200 {{ app()->getLocale() === 'en' ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-purple-50 hover:text-purple-700 border border-gray-200' }}">🇬🇧
                            EN</a>
                        <a href="{{ route('language.switch', ['locale' => 'fr']) }}"
                            class="px-2 py-2 text-xs rounded-lg text-center font-semibold transition-all duration-200 {{ app()->getLocale() === 'fr' ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-purple-50 hover:text-purple-700 border border-gray-200' }}">🇫🇷
                            FR</a>
                        <a href="{{ route('language.switch', ['locale' => 'es']) }}"
                            class="px-2 py-2 text-xs rounded-lg text-center font-semibold transition-all duration-200 {{ app()->getLocale() === 'es' ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-purple-50 hover:text-purple-700 border border-gray-200' }}">🇪🇸
                            ES</a>
                        <a href="{{ route('language.switch', ['locale' => 'pt']) }}"
                            class="px-2 py-2 text-xs rounded-lg text-center font-semibold transition-all duration-200 {{ app()->getLocale() === 'pt' ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-purple-50 hover:text-purple-700 border border-gray-200' }}">🇵🇹
                            PT</a>
                        <a href="{{ route('language.switch', ['locale' => 'sw']) }}"
                            class="px-2 py-2 text-xs rounded-lg text-center font-semibold transition-all duration-200 {{ app()->getLocale() === 'sw' ? 'bg-gradient-to-r from-purple-600 to-purple-700 text-white shadow-md' : 'bg-gray-50 text-gray-700 hover:bg-purple-50 hover:text-purple-700 border border-gray-200' }}">🇹🇿
                            SW</a>
                    </div>
                </div>
                ...
                <a href="{{ route('journals.index') }}"
                    class="block px-4 py-3 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200">
                ...
                @auth
                    <div class="border-t border-gray-100 pt-4 mt-4">
                        @if(auth()->user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}"
                                class="flex items-center px-4 py-3 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200 mb-2">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="w-full flex items-center px-4 py-3 text-sm font-semibold text-left text-gray-700 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200">
                ...
                @else
                    <div class="border-t border-gray-100 pt-4 mt-4 space-y-2">
                        <a href="{{ route('login') }}"
                            class="block px-4 py-3 text-sm font-semibold text-gray-700 hover:text-purple-700 hover:bg-purple-50 rounded-lg transition-all duration-200 text-center">
                            Sign In
                        </a>
                        <a href="{{ route('register') }}"
                            class="block px-4 py-3 text-sm font-semibold bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg transition-all duration-200 text-center shadow-md">
                            Get Started
                        </a>
                    </div>
                @endauth
    ...
    <a href="https://wa.me/27734030207" target="_blank"
        class="fixed bottom-6 right-6 z-50 bg-gradient-to-br from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-full p-4 shadow-2xl transition-all duration-300 hover:scale-110 group"
        aria-label="Contact us on WhatsApp">
        <i class="fab fa-whatsapp text-3xl group-hover:scale-110 transition-transform duration-300"></i>
        <span
            class="absolute -top-2 -right-2 w-4 h-4 bg-red-500 rounded-full border-2 border-white animate-pulse"></span>
    </a>

    <!-- Ultra Professional Footer -->
    <footer
        class="bg-gradient-to-br from-purple-900 via-purple-800 to-purple-900 text-white mt-20 border-t-4 border-purple-700">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-gray-300">
                        <li><a href="{{ route('partnership') }}"
                                class="hover:text-white transition-colors">Partnership</a></li>
                        <li><a href="{{ route('publish.index') }}" class="hover:text-white transition-colors">Author
                                Guidelines</a></li>
                        <li><a href="#" class="hover:text-white transition-colors">Newsroom</a></li>
                        <li><a href="{{ route('journals.index') }}" class="hover:text-white transition-colors">All
                                journals</a></li>
                    </ul>
                </div>
