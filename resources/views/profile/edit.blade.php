                    ...
                    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <p class="text-xs font-bold text-slate-400 opacity-80">{{ $user->email }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ tab: 'identity' }"
                    ...
                    <div class="flex flex-wrap border-b border-slate-100 bg-slate-50/50 p-2">
                        ...
                        @foreach ($tabs as $key => $tab)
                            <button type="button" @click="tab = '{{ $key }}'"
                                :class="tab === '{{ $key }}' ? 'text-cisa-base bg-white shadow-lg shadow-slate-200/50' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'"
                                ...
                                <i class="fas {{ $tab['icon'] }} text-xs"
                                    :class="tab === '{{ $key }}' ? 'text-cisa-accent' : 'text-slate-300'"></i>
                                <span>{{ $tab['label'] }}</span>
                            </button>
                        @endforeach
                    </div>
                    ...
                    <div class="p-10 space-y-10">
                        ...
                        <div x-show="tab === 'identity'" x-cloak class="animate-fadeIn">
                            ...
                            <form method="POST" action="{{ route('profile.update.identity') }}" class="space-y-8">
                                @csrf
                        ...
                        <div x-show="tab === 'contact'" x-cloak class="animate-fadeIn">
                            ...
                            <form method="POST" action="{{ route('profile.update.contact') }}" class="space-y-8">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <label
                    ...
                    <div x-show="tab === 'public'" x-cloak class="animate-fadeIn">
                        ...
                        <form method="POST" action="{{ route('profile.update.public') }}" enctype="multipart/form-data" class="space-y-8">
                            @csrf
                            ...
                            <div>
                                ...
                                <textarea name="bio" rows="5" 
                                    class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-cisa-base font-bold placeholder:text-slate-300 focus:outline-none focus:border-cisa-base focus:bg-white transition-all shadow-sm" 
                                    placeholder="Detail your research trajectories and academic expertise...">{{ old('bio', $user->bio) }}</textarea>
                            </div>
                            <div class="pt-4">
                        ...
                        <form method="POST" action="{{ route('profile.update.notifications') }}" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <label class="cursor-pointer group">
                                    <input type="checkbox" name="notify_system" value="1" @checked(old('notify_system', $user->notify_system)) class="hidden peer">
                                ...
                                <label class="cursor-pointer group">
                                    <input type="checkbox" name="notify_marketing" value="1" @checked(old('notify_marketing', $user->notify_marketing)) class="hidden peer">
                                    <div class="h-full bg-slate-50 border-2 border-slate-100 rounded-[2rem] p-8 transition-all duration-300 peer-checked:border-cisa-base peer-checked:bg-white peer-checked:shadow-2xl peer-checked:shadow-cisa-base/10 group-hover:bg-white">
                                        <div class="flex items-center justify-between mb-6">
                                            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-md border-2 border-slate-50 peer-checked:border-cisa-accent">
