...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('layout-editor.dashboard') }}" class="text-[#0056FF] hover:text-[#0044CC] inline-flex items-center mb-4">
        ...
        <div class="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
                <p class="text-sm text-blue-800">
                    ...
                    <strong>Note:</strong> Layout editing is for accepted articles that have passed copyediting. Upload final layout files (PDF, HTML, XML) for publication.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
            <h2 class="text-xl font-bold text-[#0F1B4C] mb-4 flex items-center">
                <i class="fas fa-file-pdf text-[#0056FF] mr-3"></i>Current Layout Files (Galleys)
            </h2>
                    ...
                    @foreach($submission->galleys as $galley)
                        <div class="flex items-center justify-between border-2 border-gray-200 rounded-lg p-4 hover:border-[#0056FF] transition-colors">
                            <div class="flex items-center">
                                <i class="fas fa-file-{{ $galley->type === 'pdf' ? 'pdf' : 'code' }} text-{{ $galley->type === 'pdf' ? 'red' : 'blue' }}-600 text-2xl mr-4"></i>
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        Status: <span class="font-semibold {{ $galley->approval_status === 'approved' ? 'text-green-600' : 'text-yellow-600' }}">
                                            {{ ucfirst($galley->approval_status) }}
                                        </span>
                                    </p>
                                </div>
                            </div>
            ...
            @else
                <p class="text-gray-500 text-center py-8">No layout files uploaded yet</p>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6 mb-6">
            <h2 class="text-xl font-bold text-[#0F1B4C] mb-4 flex items-center">
                <i class="fas fa-upload text-[#0056FF] mr-3"></i>Upload Layout File
            </h2>
            ...
            <form method="POST" action="{{ route('layout-editor.submissions.upload', $submission) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="label" 
                               class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-[#0056FF]"
                               placeholder="e.g., Final PDF, HTML Version">
                    </div>
                </div>
                
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        Layout File * <span class="text-gray-500">(PDF, HTML, or XML)</span>
                    </label>
                ...
                <div class="flex gap-4">
                    <a href="{{ route('layout-editor.dashboard') }}" 
                    ...
                    <button type="submit" 
                        ...
                        <i class="fas fa-upload mr-2"></i>Upload Layout File
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border-2 border-gray-200 p-6">
            <h2 class="text-xl font-bold text-[#0F1B4C] mb-4 flex items-center">
                <i class="fas fa-check-circle text-[#0056FF] mr-3"></i>Complete Layout Editing
            </h2>
            
            <p class="text-gray-700 mb-6">
                Once you've uploaded all necessary layout files, mark this submission as complete to move it to the final publishing stage.
            </p>
            
            <form method="POST" action="{{ route('layout-editor.submissions.complete', $submission) }}">
                @csrf
                <button type="submit" 
                        class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg transition-colors">
                    <i class="fas fa-check mr-2"></i>Mark Layout as Complete
                </button>
            </form>
        </div>
        @endif
