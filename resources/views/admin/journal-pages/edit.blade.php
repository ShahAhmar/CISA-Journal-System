        ...
        <div class="bg-white rounded-xl border-2 border-gray-200 p-6 space-y-6">
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg">
                <div class="flex items-start">
                    <div class="w-12 h-12 bg-blue-500 rounded-lg flex items-center justify-center text-white mr-4">
                        <i class="{{ $pageTypes[$pageType]['icon'] }} text-xl"></i>
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-bold text-blue-900 mb-1">{{ $pageData['name'] }}</h3>
                        @if($pageData['description'])
                        <p class="text-sm text-blue-700">{{ $pageData['description'] }}</p>
                        @endif
                        @if($pageTypes[$pageType]['route'])
                        <a href="{{ route($pageTypes[$pageType]['route'], $journal) }}" 
                           target="_blank"
                           class="text-sm text-blue-600 hover:text-blue-800 font-semibold mt-2 inline-flex items-center">
            ...
            <div>
                ...
                <p class="mt-2 text-xs text-gray-500">
                    <i class="fas fa-info-circle mr-1"></i>Use the toolbar above to format your content. HTML is supported.
                </p>
            </div>
