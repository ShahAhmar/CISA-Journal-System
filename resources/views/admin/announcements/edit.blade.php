...
@section('title', 'Edit Announcement - EMANP')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="type" required 
                        ...
                        <option value="new_issue" {{ old('type', $announcement->type) == 'new_issue' ? 'selected' : '' }}>New Issue</option>
                ...
                <div>
                    ...
                    <select name="journal_id" 
                        ...
                        @foreach($journals as $journal)
                            <option value="{{ $journal->id }}" {{ old('journal_id', $announcement->journal_id) == $journal->id ? 'selected' : '' }}>
                                {{ $journal->name }}
                            </option>
                        @endforeach
                    </select>
...
<div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="flex items-center">
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="send_email_notification" value="1" {{ old('send_email_notification', $announcement->send_email_notification) ? 'checked' : '' }}
                               class="w-4 h-4 text-[#0056FF] border-gray-300 rounded focus:ring-[#0056FF]">
