...
@section('title', 'Publications - Master List - ' . $journal->name)
...
@section('content')
    ...
    <div
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('journals.publications', $journal) }}" method="GET"
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="discipline"
                        ...
                        @foreach($disciplines as $d)
                            <option value="{{ $d->name }}" {{ request('discipline') == $d->name ? 'selected' : '' }}>
                                {{ $d->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap items-center justify-between mb-12 gap-4">
                <h2 class="text-2xl font-serif font-bold text-cisa-base">
                    @if(request('search') || request('year') || request('discipline'))
                        Search Results <span class="text-cisa-accent text-sm ml-2">({{ $publications->total() }} items)</span>
                    @else
                        Recent Publications
                    @endif
                </h2>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'newest']) }}"
                        class="text-xs font-bold {{ request('sort', 'newest') == 'newest' ? 'text-cisa-base border-b-2 border-cisa-accent' : 'text-slate-400 hover:text-cisa-base' }} py-1">Newest</a>
                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'oldest']) }}"
                        class="text-xs font-bold {{ request('sort') == 'oldest' ? 'text-cisa-base border-b-2 border-cisa-accent' : 'text-slate-400 hover:text-cisa-base' }} py-1 ml-4">Oldest</a>
                </div>
            </div>
            ...
            @if($publications->count() > 0)
                        ...
                        <article
                            ...
                            <div class="flex flex-col lg:flex-row gap-10">
                                ...
                                <div class="flex-grow space-y-4">
                                    <div class="flex flex-wrap items-center gap-4">
                                        <span
                                            ...
                                            {{ $submission->discipline ?? 'Interdisciplinary' }}
                                        </span>
                                        <span class="text-slate-400 text-xs font-medium flex items-center">
                                            <i class="far fa-calendar-alt mr-2 text-cisa-accent"></i>
                                            {{ $submission->published_at ? \Carbon\Carbon::parse($submission->published_at)->format('F d, Y') : 'Pending Publication' }}
                                        </span>
                                        @if($submission->doi)
                                            <span class="text-slate-300 text-xs font-mono">DOI: {{ $submission->doi }}</span>
                                        @endif
                                    </div>
                                    ...
                                    <h3
                                        ...
                                        <a
                                            href="{{ route('journals.article', ['journal' => $journal->slug, 'submission' => $submission->id]) }}">
                                            {{ $submission->title }}
                                        </a>
                                    </h3>
                                ...
                                <div
                                    ...
                                    <a href="{{ route('journals.article', ['journal' => $journal->slug, 'submission' => $submission->id]) }}"
                ...
                <!-- Pagination -->
                <div class="mt-20">
                    {{ $publications->appends(request()->query())->links('vendor.pagination.tailwind') }}
                </div>
            @else
