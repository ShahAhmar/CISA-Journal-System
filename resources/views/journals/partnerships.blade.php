...
@section('title', 'Partnerships - ' . $journal->name)
    ...
    <div class="py-12 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="max-w-4xl mx-auto space-y-12">
                ...
                @if($journal->partnerships_content)
                            ...
                            @foreach($journal->partners as $partner)
                                <div class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden">
                                    ...
                                    @if($partner->website_url)
                                        <a href="{{ $partner->website_url }}" target="_blank" class="flex flex-col items-center">
                                    @endif
                                    
                                    <img src="{{ asset('storage/' . $partner->logo) }}" alt="{{ $partner->name }}" class="h-20 w-auto object-contain mb-4 filter grayscale group-hover:grayscale-0 transition-all duration-500">
                                    
                                    @if($partner->name)
                                        <span class="text-xs font-bold text-gray-500 uppercase tracking-widest text-center">{{ $partner->name }}</span>
                                    @endif
                ...
                @if(!$journal->partnerships_content && $journal->partners->count() == 0)
