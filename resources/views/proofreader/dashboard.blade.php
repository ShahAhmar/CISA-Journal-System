...
@section('title', 'Proofreader Dashboard | CISA')
    ...
    <div class="min-h-screen bg-slate-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-center justify-between">
                    ...
                    <div class="hidden md:block">
                        <div
                            ...
                            <i class="fas fa-check-double text-4xl text-cisa-accent"></i>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <!-- Pending -->
                <a href="{{ route('proofreader.dashboard', ['status' => 'pending']) }}"
                ...
                <!-- In Progress -->
                <a href="{{ route('proofreader.dashboard', ['status' => 'in_progress']) }}"
                ...
                <!-- Completed -->
                <a href="{{ route('proofreader.dashboard', ['status' => 'completed']) }}"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                        ...
                                        <td class="px-6 py-4">
                                            <span
                                                class="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider
                                                    {{ $submission->current_stage === 'proofreading' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600' }}">
                                                {{ ucfirst($submission->current_stage) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-xs text-gray-500 font-mono">
                                            {{ $submission->proofreading_due_date ? $submission->proofreading_due_date->format('M d, Y') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <a href="{{ route('proofreader.submissions.show', $submission) }}"
                ...
                @else
                    <div class="p-16 text-center">
                        ...
                        <p class="text-gray-400 text-sm">You haven't been assigned any proofreading tasks yet.</p>
                    </div>
                @endif
            </div>
        </div>
