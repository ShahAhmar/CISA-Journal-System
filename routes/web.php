...
Route::get('/journal/{journal:slug}/about', [JournalController::class, 'about'])->name('journals.about');
Route::get('/journal/{journal:slug}/info', [JournalController::class, 'info'])->name('journals.info');
Route::get('/journal/{journal:slug}/publications', [JournalController::class, 'publications'])->name('journals.publications');
Route::get('/journal/{journal:slug}/call-for-papers', [JournalController::class, 'callForPapers'])->name('journals.call-for-papers');
Route::get('/journal/{journal:slug}/apc-submission', [JournalController::class, 'apcSubmission'])->name('journals.apc-submission');
Route::get('/journal/{journal:slug}/editorial-ethics', [JournalController::class, 'editorialEthics'])->name('journals.editorial-ethics');
Route::get('/journal/{journal:slug}/partnerships', [JournalController::class, 'partnerships'])->name('journals.partnerships');
...
// 301 Redirects for Consolidated/Retired Pages
Route::redirect('/journal/{journal:slug}/aims-scope', '/journal/{journal}/about', 301);
Route::redirect('/journal/{journal:slug}/history', '/journal/{journal}/about', 301);
Route::redirect('/journal/{journal:slug}/vision', '/journal/{journal}/about', 301);
Route::redirect('/journal/{journal:slug}/mission', '/journal/{journal}/about', 301);
Route::redirect('/journal/{journal:slug}/issues', '/journal/{journal}/publications', 301);
Route::redirect('/journal/{journal:slug}/archives', '/journal/{journal}/publications', 301);
Route::redirect('/journal/{journal:slug}/latest-issue', '/journal/{journal}/publications', 301);
Route::redirect('/journal/{journal:slug}/editorial-board', '/journal/{journal}/editorial-ethics', 301);
Route::redirect('/journal/{journal:slug}/peer-review-policy', '/journal/{journal}/editorial-ethics', 301);
Route::redirect('/journal/{journal:slug}/open-access-policy', '/journal/{journal}/editorial-ethics', 301);
Route::redirect('/journal/{journal:slug}/editorial-policies', '/journal/{journal}/editorial-ethics', 301);
Route::redirect('/journal/{journal:slug}/copyright-notice', '/journal/{journal}/editorial-ethics', 301);
Route::redirect('/journal/{journal:slug}/submission-guidelines', '/journal/{journal}/apc-submission', 301);
Route::redirect('/journal/{journal:slug}/author-guidelines', '/journal/{journal}/apc-submission', 301);

Route::get('/journal/{journal:slug}/issue/{issue}', [JournalController::class, 'showIssue'])->name('journals.issue');
Route::get('/journal/{journal:slug}/issue/{issue}', [JournalController::class, 'showIssue'])->name('journals.issue');
Route::get('/journal/{journal:slug}/article/{submission}', [JournalController::class, 'showArticle'])
    ->where(['submission' => '[0-9]+'])
    ->name('journals.article');
...
Route::get('/journal/{journal:slug}/article/{submission}/view', [JournalController::class, 'viewArticleFile'])->name('journals.article.view');
...
// Public Metadata Export Routes (for published articles)
...
Route::middleware('auth')->group(function () {
    ...
    // MUST be before /submissions/{submission} route to avoid route conflicts
    Route::get('/submissions/{submission}/files/{file}/download', [\App\Http\Controllers\FileDownloadController::class, 'download'])
        ->where(['submission' => '[0-9]+', 'file' => '[0-9]+'])
        ->name('submissions.files.download');

    Route::get('/submissions/{submission}/galleys/{galley}/download', [\App\Http\Controllers\FileDownloadController::class, 'downloadGalley'])
        ->where(['submission' => '[0-9]+', 'galley' => '[0-9]+'])
        ->name('submissions.galleys.download');
    ...
    Route::prefix('author')->name('author.')->group(function () {
        ...
        // Copyedit approval routes - GET redirects back, POST performs action
        Route::get('/submissions/{submission}/copyedit/approve', function (\App\Models\Submission $submission) {
            return redirect()->route('author.submissions.show', $submission)
                ->with('info', 'Please use the approve button in the form to approve copyedited files.');
        });
        ...
        Route::get('/submissions/{submission}/copyedit/request-changes', function (\App\Models\Submission $submission) {
            return redirect()->route('author.submissions.show', $submission)
                ->with('info', 'Please use the request changes form to request changes to copyedited files.');
        });
        ...
        // Author Payments
        Route::get('/payments', [\App\Http\Controllers\Author\PaymentController::class, 'index'])->name('payments.index');
        Route::get('/payments/{payment}', [\App\Http\Controllers\Author\PaymentController::class, 'show'])->name('payments.show');
        Route::post('/payments/{payment}/proof', [\App\Http\Controllers\Author\PaymentController::class, 'uploadProof'])->name('payments.proof');
    });
    ...
    Route::prefix('editor')->name('editor.')->group(function () {
        ...
        // Request revision (fallback without slug; GET will redirect back)
        Route::match(['get', 'post'], '/submissions/{submission}/request-revision', [EditorController::class, 'requestRevision'])->name('submissions.request-revision.no-slug');
        ...
        // Payments Management for Editors
        Route::get('/journal/{journal:slug}/payments', [EditorController::class, 'payments'])->name('payments.index');
        Route::get('/journal/{journal:slug}/payments/{payment}', [EditorController::class, 'showPayment'])->name('payments.show');
        Route::put('/journal/{journal:slug}/payments/{payment}/status', [EditorController::class, 'updatePaymentStatus'])->name('payments.update-status');
        Route::put('/journal/{journal:slug}/payments/{payment}', [EditorController::class, 'updatePayment'])->name('payments.update');
    ...
    // Plagiarism Check (Universal)
    Route::post('/submissions/{submission}/plagiarism/check', [\App\Http\Controllers\Admin\PlagiarismController::class, 'runCheck'])->name('submissions.plagiarism.check');
    Route::get('/submissions/{submission}/plagiarism/report', [\App\Http\Controllers\Admin\PlagiarismController::class, 'showReport'])->name('submissions.plagiarism.report');
    ...
    Route::prefix('copyeditor')->name('copyeditor.')->group(function () {
        ...
        Route::get('/submissions/{submission}/file/{file}/download', [\App\Http\Controllers\CopyeditorController::class, 'downloadFile'])->name('submissions.file.download');
        // If someone opens the upload URL with GET directly, redirect back to submission page
        Route::get('/submissions/{submission}/upload', function (\App\Models\Submission $submission) {
            return redirect()->route('copyeditor.submissions.show', $submission)
                ->with('info', 'Please use the upload form to submit the copyedited file.');
        });
    ...
    Route::prefix('proofreader')->name('proofreader.')->group(function () {
        ...
        Route::get('/submissions/{submission}/file/{file}/download', [\App\Http\Controllers\ProofreaderController::class, 'downloadFile'])->name('submissions.file.download');
    ...
    Route::prefix('admin')->name('admin.')->middleware('can:access-admin')->group(function () {
        ...
        Route::post('submissions/{submission}/disable', [\App\Http\Controllers\Admin\SubmissionController::class, 'disable'])->name('submissions.disable');
        Route::post('submissions/{submission}/enable', [\App\Http\Controllers\Admin\SubmissionController::class, 'enable'])->name('submissions.enable');
        ...
        Route::post('users/{user}/terminate', [\App\Http\Controllers\Admin\UserController::class, 'terminate'])->name('users.terminate');
        Route::post('users/{user}/restore-terminated', [\App\Http\Controllers\Admin\UserController::class, 'restoreTerminated'])->name('users.restore-terminated');
        ...
        Route::get('/website-settings/header-footer', [\App\Http\Controllers\Admin\WebsiteSettingsController::class, 'headerFooter'])->name('website-settings.header-footer');
        Route::post('/website-settings/header-footer', [\App\Http\Controllers\Admin\WebsiteSettingsController::class, 'updateHeaderFooter'])->name('website-settings.update-header-footer');
        ...
        Route::get('/visual-editor/{id}', [\App\Http\Controllers\Admin\VisualEditorController::class, 'edit'])->name('visual-editor.edit');
        Route::post('/visual-editor/{id}/save', [\App\Http\Controllers\Admin\VisualEditorController::class, 'store'])->name('visual-editor.store');
