...
<?php $__env->startSection('title', 'Submit Article - ' . $journal->name . ' | EMANP'); ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="text-center">
            <h1 class="text-3xl md:text-4xl font-bold mb-2" style="font-family: 'Playfair Display', serif; font-weight: 700;">
                Submit Article to <?php echo e($journal->name); ?>

            </h1>
            <p class="text-lg text-amber-200">Follow the steps below to submit your manuscript</p>
        </div>
    </div>
</section>
...
<div class="bg-white border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav class="flex items-center space-x-2 text-sm">
            <a href="<?php echo e(route('journals.index')); ?>" class="text-cisa-accent hover:text-amber-600 transition-colors">
                <i class="fas fa-home"></i> Home
            </a>
            <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
            <a href="<?php echo e(route('journals.show', $journal)); ?>" class="text-cisa-accent hover:text-amber-600 transition-colors">
                <?php echo e(Str::limit($journal->name, 30)); ?>
...
">
    <div id="submission-form-container" class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center flex-1">
                            <div class="flex items-center">
                                <div class="flex items-center justify-center w-10 h-10 rounded-full border-2 transition-colors"
                                     :class="currentStep > index ? 'bg-cisa-accent border-cisa-accent text-white' : (currentStep === index ? 'border-cisa-accent text-cisa-accent' : 'border-gray-300 text-gray-400')">
                                ...
                                <span class="ml-3 font-semibold text-sm hidden md:block"
                                      :class="currentStep >= index ? 'text-cisa-base' : 'text-gray-400'"
                                      x-text="step.title"></span>
                            </div>
                            <div x-show="index < steps.length - 1" class="flex-1 mx-4 hidden md:block">
                                <div class="h-1 rounded-full"
                                     :class="currentStep > index ? 'bg-cisa-accent' : 'bg-gray-200'"></div>
                            </div>
                        </div>
                    </template>
                </div>
        ...
        <form method="POST" action="<?php echo e(route('author.submissions.store', $journal)); ?>" enctype="multipart/form-data" id="submissionForm" @submit.prevent="submitForm">
            <?php echo csrf_field(); ?>
            ...
            <div x-show="currentStep === 0" class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-8 md:p-12">
                <h2 class="text-2xl font-bold text-cisa-base mb-6">Step 1: Start Submission</h2>
                
                <!-- Section Selection -->
                <div class="mb-8">
                    <label class="block text-sm font-bold text-cisa-base mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer transition-all hover:border-[#0056FF] hover:bg-blue-50"
                               :class="formData.journal_section_id == <?php echo e($section->id); ?> ? 'border-cisa-accent bg-amber-50' : 'border-gray-200'">
                            <input type="radio" name="journal_section_id" value="<?php echo e($section->id); ?>" 
                                   x-model="formData.journal_section_id"
                                   <?php echo e(old('journal_section_id') == $section->id ? 'checked' : ''); ?>

                                   class="mr-3">
                            <div class="flex-1">
                                <div class="font-semibold text-cisa-base"><?php echo e($section->title); ?></div>
                                <?php if($section->description): ?>
                                <div class="text-sm text-gray-600 mt-1"><?php echo e($section->description); ?></div>
                                <?php endif; ?>
                                <?php if($section->word_limit_min || $section->word_limit_max): ?>
                                <div class="text-xs text-gray-500 mt-1">
                                    Word limit: 
                                    <?php if($section->word_limit_min): ?><?php echo e($section->word_limit_min); ?><?php endif; ?>
                                    <?php if($section->word_limit_min && $section->word_limit_max): ?> - <?php endif; ?>
                                    <?php if($section->word_limit_max): ?><?php echo e($section->word_limit_max); ?><?php endif; ?>
                                </div>
                                <?php endif; ?>
                            </div>
                        </label>
                    ...
                    <?php $__errorArgs = ['journal_section_id'];
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="mb-8">
                    ...
                    <div class="bg-[#F7F9FC] border-2 border-gray-200 rounded-lg p-6 mb-4">
                        <?php if(!empty($submissionRequirements) || !empty($submissionChecklist)): ?>
                            <?php if(!empty($submissionRequirements)): ?>
                        <div class="mb-4">
                            <h4 class="font-semibold text-cisa-base mb-2">Requirements:</h4>
                            <div class="prose max-w-none text-sm">
                                <?php echo $submissionRequirements; ?>

                            </div>
                        </div>
                        <?php endif; ?>
                            <?php if(!empty($submissionChecklist)): ?>
                        <div>
                            <h4 class="font-semibold text-cisa-base mb-2">Checklist:</h4>
                            <div class="prose max-w-none text-sm">
                                <?php echo $submissionChecklist; ?>
                    ...
                    <label class="flex items-start">
                        <input type="checkbox" name="requirements_accepted" value="1" required
                               ...
                               <?php echo e(old('requirements_accepted') ? 'checked' : ''); ?>
                    ...
                    <?php $__errorArgs = ['requirements_accepted'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
                ...
                <!-- Payment Choice -->
                <div class="mb-8 p-6 bg-slate-50 border-2 border-slate-200 rounded-2xl">
                    <label class="block text-sm font-black text-cisa-base uppercase tracking-widest mb-4">
                        Payment Preference <span class="text-red-500">*</span>
                    </label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:bg-white"
                               :class="formData.is_pay_later === '0' ? 'border-cisa-accent bg-white shadow-md' : 'border-gray-200'">
                            <input type="radio" name="is_pay_later" value="0" x-model="formData.is_pay_later" class="mr-3">
                            <div>
                                <div class="font-bold text-cisa-base">Pay Now</div>
                                <div class="text-xs text-slate-500">I will pay the APC/Submission fee now</div>
                            </div>
                        </label>
                        <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all hover:bg-white"
                               :class="formData.is_pay_later === '1' ? 'border-cisa-accent bg-white shadow-md' : 'border-gray-200'">
                            <input type="radio" name="is_pay_later" value="1" x-model="formData.is_pay_later" class="mr-3">
                            <div>
                                <div class="font-bold text-cisa-base">Pay Later</div>
                                <div class="text-xs text-slate-500">I will provide payment proof after editor's acceptance</div>
                            </div>
                        </label>
                    </div>
                    <p class="mt-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                        <i class="fas fa-info-circle mr-1"></i> Selecting "Pay Later" allows you to submit without immediate payment. However, publication will only proceed after payment confirmation.
                    </p>
                </div>
                ...
                <div class="flex justify-end mt-8">
                    <button type="button" @click="nextStep()" 
                            ...
                            class="px-8 py-3 bg-cisa-accent hover:bg-amber-600 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div x-show="currentStep === 1" class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-8 md:p-12">
                <h2 class="text-2xl font-bold text-cisa-base mb-6">Step 2: Upload Submission Files</h2>
                ...
                <div class="mb-6">
                    ...
                    <div class="border-2 border-dashed rounded-lg p-6 hover:border-cisa-accent transition-colors bg-slate-50 <?php echo e($errors->has('manuscript') ? 'border-red-500' : 'border-gray-300'); ?>">
                        <input type="file" name="manuscript" id="manuscript" required 
                               ...
                               class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cisa-accent file:text-white hover:file:bg-amber-600 file:cursor-pointer cursor-pointer">
...
unset($__errorArgs, $__bag); ?>
                ...
                <?php if($journal->review_method === 'double_blind'): ?>
                <div class="mb-6">
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-[#0056FF] transition-colors bg-[#F7F9FC]">
                        <input type="file" name="title_page" id="title_page" 
                        ...
                        <p class="text-sm text-gray-500 mt-2">
                            <i class="fas fa-info-circle mr-1"></i>Remove author names and affiliations for blind review. Accepted formats: DOC, DOCX, PDF
                        </p>
                ...
                <div class="mb-6">
                    ...
                    <div class="border-2 border-dashed rounded-lg p-6 hover:border-[#0056FF] transition-colors bg-[#F7F9FC] <?php echo e($errors->has('tables.*') || $errors->has('tables.0') ? 'border-red-500' : 'border-gray-300'); ?>">
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="flex justify-between mt-8">
                    <button type="button" @click="prevStep()" 
                            class="px-8 py-3 bg-white hover:bg-gray-50 text-cisa-base border-2 border-cisa-base rounded-lg font-semibold transition-colors">
            ...
            <div x-show="currentStep === 2" class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-8 md:p-12">
                <h2 class="text-2xl font-bold text-cisa-base mb-6">Step 3: Enter Metadata</h2>
                
                <div class="space-y-6">
                    <!-- Title -->
                    <div>
                        <label for="title" class="block text-sm font-bold text-cisa-base mb-2">
                            Title of Paper <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="title" name="title" required 
                               x-model="formData.title"
                               value="<?php echo e(old('title', '')); ?>"
                               class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-cisa-accent transition-colors text-gray-900 <?php echo e($errors->has('title') ? 'border-red-500' : ''); ?>"
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <textarea id="abstract" name="abstract" rows="8" required
                                  ...
                                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-[#0056FF] transition-colors resize-none text-gray-900 <?php echo e($errors->has('abstract') ? 'border-red-500' : ''); ?>"
                                  placeholder="Provide a comprehensive abstract of your article"><?php echo e(old('abstract', '')); ?></textarea>
                        <div class="text-xs text-gray-500 mt-1">
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                            ...
                            <template x-for="(author, index) in formData.authors" :key="index">
                                <div class="author-entry bg-[#F7F9FC] border-2 border-gray-200 rounded-xl p-6">
                                    <div class="flex justify-between items-center mb-4">
                                        <h4 class="font-semibold text-cisa-base">Author <span x-text="index + 1"></span></h4>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <input type="text" 
                                                   ...
                                                   required
                                                   class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-cisa-accent transition-colors text-gray-900">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                        ...
                        <button type="button" @click="addAuthor()" 
                                class="mt-4 bg-slate-50 hover:bg-cisa-accent hover:text-white text-cisa-accent border-2 border-cisa-accent px-6 py-2.5 rounded-lg font-semibold transition-colors text-sm">
                    ...
                    <div>
                        ...
                        <textarea id="supporting_agencies" name="supporting_agencies" rows="4"
                                  ...
                                  placeholder="List any funding agencies, grants, or supporting organizations"><?php echo e(old('supporting_agencies', '')); ?></textarea>
                    </div>
                </div>
            ...
            <div x-show="currentStep === 3" class="bg-white rounded-2xl shadow-xl border-2 border-gray-100 p-8 md:p-12">
                <h2 class="text-2xl font-bold text-cisa-base mb-6">Step 4: Review & Confirm</h2>
                
                <div class="space-y-6">
                    <!-- Section -->
                    <div class="bg-[#F7F9FC] rounded-lg p-4">
                        <h3 class="font-semibold text-cisa-base mb-2">Section</h3>
...
function submissionForm() {
    ...
    const oldData = <?php echo json_encode(old(), 15, 512) ?>;
    const oldAuthors = <?php echo json_encode(old('authors', []), 512) ?>;
    ...
    let authors = [
        {
            first_name: oldData.authors?.[0]?.first_name || '<?php echo e(auth()->user()->first_name); ?>',
            last_name: oldData.authors?.[0]?.last_name || '<?php echo e(auth()->user()->last_name); ?>',
            email: oldData.authors?.[0]?.email || '<?php echo e(auth()->user()->email); ?>',
            country: oldData.authors?.[0]?.country || '',
            affiliation: oldData.authors?.[0]?.affiliation || '<?php echo e(auth()->user()->affiliation ?? ''); ?>',
            orcid: oldData.authors?.[0]?.orcid || '<?php echo e(auth()->user()->orcid ?? ''); ?>'
        }
    ];
    ...
    return {
        ...
        formData: {
            ...
            is_pay_later: oldData.is_pay_later || '0',
            title: oldData.title || '',
        ...
        canProceed(step) {
            if (step === 0) {
                return this.formData.journal_section_id && 
                       ...
                       this.formData.is_pay_later !== null;
            }
            if (step === 1) {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u857061584/domains/cisajournal.org/public_html/resources/views/author/submissions/create-multistep.blade.php ENDPATH**/ ?>